...
///
/// A widget that renders an Jxl with [AnimationController].
///
@immutable
class JxlAnimation extends StatefulWidget {
  /// Rendered image cache.
  static AnimationCache cache = AnimationCache();

  /// [ImageProvider] of this image. Like [FileJxlImage], [JxlNetworkImage], [JxlAssetImage], [JxlMemoryImage]
  ...
  /// Creates a widget that displays a controllable Jxl.
  const JxlAnimation({
    Key? key,
    required this.image,
    this.controller,
  ...
  @override
  State<JxlAnimation> createState() => _JxlAnimationState();
}

///
/// Works as a cache system for already fetched [JxlAnimationInfo].
///
@immutable
class AnimationCache {
  final Map<String, JxlAnimationInfo> caches = {};
...
///
/// Stores all the [ImageInfo] and duration of an Jxl.
///
@immutable
class JxlAnimationInfo {
  final List<ImageInfo> frames;
  final Duration duration;

  const JxlAnimationInfo({
    required this.frames,
    required this.duration,
  });
}

class _JxlAnimationState extends State<JxlAnimation>
    with SingleTickerProviderStateMixin {
  late final AnimationController _controller;

  /// List of [ImageInfo] of every frame of this Jxl.
  ...
  @override
  void didUpdateWidget(JxlAnimation oldWidget) {
  ...
  ///
  /// The calculation is based on the frames of the Jxl
  ...
  Future<void> _loadFrames() async {
    ...
    JxlAnimationInfo Jxl = widget.useCache
        ? JxlAnimation.cache.caches.containsKey(_getImageKey(widget.image))
            ? JxlAnimation.cache.caches[_getImageKey(widget.image)]!
    ...
    if (widget.useCache) {
      JxlAnimation.cache.caches
          .putIfAbsent(_getImageKey(widget.image), () => Jxl);
    }

    setState(() {
      _frames = Jxl.frames;
      _controller.duration = _controller.duration ?? Jxl.duration;
  ...
  /// Fetches single Jxl frames and saves them into the [AnimationCache] of [JxlAnimation]
  static Future<JxlAnimationInfo> _fetchFrames(ImageProvider provider) async {
    late final Uint8List bytes;

    if (provider is NetworkJxlImage) {
    ...
    } else if (provider is AssetJxlCodecImage) {
      bytes = (await (provider.bundle ?? rootBundle).load(provider.asset))
          .buffer
          .asUint8List();
    } else if (provider is FileJxlImage) {
      bytes = await provider.file.readAsBytes();
    } else if (provider is MemoryJxlImage) {
      bytes = provider.bytes;
    }

    JxlCodec codec = MultiFrameJxlCodec(
    ...
    for (int i = 0; i < codec.frameCount; i++) {
      JxlFrameInfo frameInfo = await codec.getNextFrame();
      infos.add(ImageInfo(image: frameInfo.image));
      duration += frameInfo.duration;
    }
    codec.dispose();

    return JxlAnimationInfo(frames: infos, duration: duration);
  }
}
